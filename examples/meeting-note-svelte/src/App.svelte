<script lang="ts">
  ...
  let note = exampleMeetingNote;
  let status: ChatStatus = "idle";
  let prompt = defaultPrompt;
  let details: PromptResponseDetails | undefined;
  let stream: StreamState | undefined;
  let meetingRuntime: MeetingRuntime | undefined;
  let errorMessage = "";
  let showDetails = false;
  let canSend = true;
  let hasAssistantResult = false;
  let assistantRenderer: Snippet | undefined;

  $: canSend = note.trim().length > 0 && status === "idle";
    ...
    try {
      ...
      for await (const event of mockMeetingActionsAgentStream({
        ...
        if (errorDiagnostics.length > 0) {
          status = "error";
          errorMessage = errorDiagnostics.map((diagnostic) => diagnostic.message).join("\n");
          return;
        }
  ...
  <section class="grid h-full min-h-0 gap-6 lg:grid-cols-[minmax(0,1fr)_440px]">
    ...
    >
      ...
      <div class="flex min-h-0 flex-1 flex-col gap-3 px-6">
        <div aria-live="polite" class="flex min-h-0 flex-1 flex-col gap-3 overflow-auto empty:hidden">
          {#if hasAssistantResult}
            <div class="ml-auto max-w-[86%] rounded-lg bg-primary px-3 py-2 text-sm text-primary-foreground">
          ...
          {:else if assistantRenderer}
            <div class="mr-auto w-full max-w-[92%]">
              {@render assistantRenderer()}
            </div>
          {/if}
        </div>

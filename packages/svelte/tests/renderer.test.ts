...
test("renders the verified root node through a registry render binding", () => {
  ...
  const html = renderWasserToHtml({ ui, registry });
    ...
    components: {
      Layout: {
        ...
        render: (({ children, slots }) =>
          wasserElement("section", undefined, [
            wasserElement("header", undefined, slots.header),
            wasserElement("div", undefined, children),
          ])) satisfies WasserRenderBinding,
      },
      Text: {
        description: "Text node.",
        render: (({ props }) =>
          wasserElement("p", undefined, String(props.label))) satisfies WasserRenderBinding<{
          label: string;
        }>,
      },
    },
...
test("exposes node, slot, and stream renderer functions", () => {
  const registry = createWasserRegistry({
    components: {
      Layout: {
        ...
        render: (({ slots }) =>
          wasserElement("section", undefined, slots.header)) satisfies WasserRenderBinding,
      },
      Text: {
        description: "Text node.",
        render: (({ props }) =>
  ...
  const nodeHtml = renderWasserNodeToHtml({ ui, registry, nodeId: "title" });
  const slotHtml = renderWasserSlotToHtml({ ui, registry, nodeId: "layout", name: "header" });
  const streamHtml = renderWasserStreamToHtml({ ui, registry });
...
test("renders verified stream state through renderWasserStreamToHtml", () => {
  const registry = createWasserRegistry({
    components: {
      Page: {
        ...
        children: "nodes",
        render: (({ children }) =>
          wasserElement("main", undefined, children)) satisfies WasserRenderBinding,
      },
      Text: {
        description: "Text node.",
        render: (({ props }) =>
  ...
  const html = renderWasserStreamToHtml({ stream, registry });
...
test("binds runtime data, actions, and telemetry into render context", () => {
  ...
  const registry = createWasserRegistry({
    components: {
      App: {
        ...
        children: "nodes",
        render: (({ children }) =>
          wasserElement("div", undefined, children)) satisfies WasserRenderBinding,
      },
      CustomerTable: {
        ...
        render: ((context) => {
          ...
          return wasserElement(
            "table",
            undefined,
            wasserElement("caption", undefined, rows.join(", ")),
          );
        }) satisfies WasserRenderBinding,
      },
      ExportButton: {
  ...
  const html = renderWasserToHtml({ ui, registry, runtime });
  ...
  const registry = createWasserRegistry({
    components: {
      AdminPanel: {
        ...
        render: (() => {
          called = true;
          return wasserElement("section", undefined, "Admin");
        }) satisfies WasserRenderBinding,
      },
    },
  });
...
test("renders safe fallbacks for missing render bindings and raw schema input", () => {
  ...
  const missingBindingHtml = renderWasserToHtml({
    ui,
    registry,
  ...
  const rawHtml = renderWasserToHtml({
    ui: {
      kind: "wasser.ui.document",
      version: "wasser.ui.v1",
      root: "metric",
